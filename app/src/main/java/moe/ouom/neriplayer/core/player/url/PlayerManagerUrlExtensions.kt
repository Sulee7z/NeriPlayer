...
internal suspend fun PlayerManager.resolveShareableListenTogetherStreamUrls(
    ...
    val resolution = when {
        ...
        isQQMusicTrack(song) -> resolveCustomPlatformListenTogetherStreams(song) {
            getQQMusicAudioUrl(song = song, suppressError = true)
        }
        isKugouTrack(song) -> resolveCustomPlatformListenTogetherStreams(song) {
            getKugouAudioUrl(song = song, suppressError = true)
        }
        isKuWoTrack(song) -> resolveCustomPlatformListenTogetherStreams(song) {
            getKuwoAudioUrl(song = song, suppressError = true)
        }
...
/**
 * 用户扩展平台(QQ/酷狗/酷我)的一起听共享流解析: 复用各平台单 URL 解析,
 * 包装成上游的多候选结构(单候选)
 */
private suspend fun PlayerManager.resolveCustomPlatformListenTogetherStreams(
    song: SongItem,
    resolve: suspend () -> SongUrlResult
): ShareableListenTogetherStreamResolution {
    val result = runCatching { resolve() }.getOrNull()
    val url = (result as? SongUrlResult.Success)?.url?.takeIf { isDirectStreamUrl(it) }
    return ShareableListenTogetherStreamResolution(
        streamUrls = listOfNotNull(url),
        isPreviewOnly = false
    )
}
