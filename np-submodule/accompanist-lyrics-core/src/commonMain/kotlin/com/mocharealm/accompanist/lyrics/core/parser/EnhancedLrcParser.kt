...
object EnhancedLrcParser : ILyricsParser {
    ...
    private fun List<ISyncedLine>.combineRawWithTranslation(): List<ISyncedLine> {
        ...
        for (i in this.indices) {
            ...
            // 制作信息/元数据行 (作词/作曲/OP 等) 不参与原文↔译文配对:
            // 既不去"窃取"后面的正文行当译文 (否则同/近时间戳时会整体错位一行)
            // 也不会被当作某行的译文显示
            ...
            for (j in i + 1 until this.size) {
                ...
                if (isCompatibleType && abs(line.start - nextLine.start) <= 150) {
                    ...
                    // 译文候选不能是制作信息行
                    if (LrcMetadataHelper.isCreditLine(nextContent)) {
                        continue
                    }
    ...
    private fun List<ISyncedLine>.rearrangeUncheckedLineTime(): List<ISyncedLine> =
        this.mapIndexed { index, line ->
            if (line is SyncedLine) {
                val nextStart = this.getOrNull(index + 1)?.start ?: Int.MAX_VALUE
                // 容错: 下一行时间戳早于当前行 (乱序) 时钳制 end 不小于 start
                // 保证该行仍是有效可高亮区间而非触发构造异常被整体丢弃
                val end = maxOf(line.start, nextStart)
                line.copy(end = end)
            } else {
                line
            }
